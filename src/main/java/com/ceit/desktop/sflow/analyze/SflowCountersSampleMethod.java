...
import com.ceit.desktop.sflow.beans.*;
...
public class SflowCountersSampleMethod {
	...
	public static void getCountersSampleData(DataInputStream dis, SflowHeader sflowHeader, SflowCountSample sflowCountSample) throws IOException {
		StringBuffer str = new StringBuffer();
		/************** expanded counters sample *********************/
		ExpandedCountersSampleHeader countersSampleHeader = getExpandedCountersSampleHeader(dis);
		int i = countersSampleHeader.getNum_elements();
		int seqNumber = countersSampleHeader.getSequence_number1();
		str.append(sflowHeader + ","+countersSampleHeader + ",");
		// 根据元素的个数进行处理
		while (i > 0) {
			/** 处理元素的方法 **/
			/** sflow_extendedcounter_generalif_data **/
			Object counter = getExpandedCountersSampleDate(dis,sflowCountSample);
			if (null != counter) {
				str.append(counter.toString()+",");
			}
			i--;
		...
		if(countersSampleHeader.getNum_elements()==1){
			...
			sflowCountSample.setSeqNumber(seqNumber);
			str.append("\"dataType\":\"CountersSample\"}");
		}
		//System.out.println(str);
	}
	...
	public static Object getExpandedCountersSampleDate(DataInputStream dis, SflowCountSample sflowCountSample) throws IOException {
		int tag = dis.readInt();// 读取类型tag，1、2、1101
		// 1、代表Generic Interface Counters字段
		if (tag == 1) {
			int length = dis.readInt();// 总字节长度（不包含tag和length）
			...
			sflowCountSample.setIfType(ifType);
			sflowCountSample.setIfIndex(ifIndex);

			sflowCountSample.setInOct(ifInOctets);
			sflowCountSample.setInPkt(ifInUcastPkts);
			sflowCountSample.setInMtcPkt(ifInMulticastPkts);
			sflowCountSample.setInBroPkt(ifInBroadcastPkts);
			sflowCountSample.setOutOct(ifOutOctets);
			sflowCountSample.setOutPkt(ifOutUcastPkts);
			sflowCountSample.setOutMtcPkt(ifOutMulticastPkts);
			sflowCountSample.setOutBroPkt(ifOutBroadcastPkts);
